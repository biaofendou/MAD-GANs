...
"""
为了更好地了解堆叠LSTM模型与我们提出的MSLSTM之间的差异，我们还设计了一个实验以比较堆叠模型和我们的模型的重建结果。
通过将输出标签替换为原始序列，并使用平方损失作为目标函数，MSLSTM和堆叠的LSTM对输入的重构可以揭示学习表示的有效性。
如图14所示，MSLSTM和堆叠模型可以学习流量数据的全局演变。
但是两者相比，MSLSTM捕获所有必要的详细突发性（x = 571和105），堆叠LSTM无法在那些情况下保留突发性位置，而仅重建原始信号的概图。
观察结果表明，堆叠的LSTM比我们的模型更容易欠拟合数据，因为它仅从原始信号中生成更高级别的时间概况，并且无法同时考虑多尺度信息。
在MSLSTM中，提取的大量信息被充分利用以学习本地特征和全局趋势。
"""
def set_style():
...
def epoch_acc_plotting(filename,case_,sequence_window,learning_rate,train_acc_,val_acc_):
    if not os.path.isdir(os.path.join(os.getcwd(),'picture')):
        os.makedirs(os.path.join(os.getcwd(),'picture'))
    epoch = len(train_acc_[0])
    color_ = ['y', 'g','#FF8C00','#FD8CD0','c', 'b', 'r', 'm']
    #color_ = ['y', 'g', 'c', 'b', 'r', 'm']

    x = [i+1 for i in range(epoch)]
    plt.figure()
    for tab in range(len(case_)):
        plt.plot(x,train_acc_[tab],color_[tab],label=case_[tab] + ' train acc')
        plt.plot(x, val_acc_[tab], color_[len(case_)+tab],label=case_[tab] +' val acc')
...
def epoch_loss_plotting(filename,case_,sequence_window,learning_rate,train_loss_,val_loss_):
    if not os.path.isdir(os.path.join(os.getcwd(),'picture')):
        os.makedirs(os.path.join(os.getcwd(),'picture'))
    epoch = len(train_loss_[0])
    color_ = ['y', 'g','#FF8C00','#FD8CD0','c', 'b', 'r', 'm']

    x = [i+1 for i in range(epoch)]
    plt.figure()
    for tab in range(len(case_)):
        plt.plot(x,train_loss_[tab],color_[tab],label=case_[tab] + ' train loss')
        plt.plot(x, val_loss_[tab], color_[len(case_)+tab],label=case_[tab] +' val loss')
...
def weight_plotting(filename,sequence_window,corss_val_label,learning_rate,weight_):
    if not os.path.isdir(os.path.join(os.getcwd(),'picture')):
        os.makedirs(os.path.join(os.getcwd(),'picture'))
    weight__new = np.transpose(weight_)
    #a,b,c = weight__new.shape
    subtitle = ['a', 'b', 'c', 'd', 'e', 'f','g','h','i','j']

    X = [i for i in range(len(weight__new[0][0]))]
    ...
    for tab in range(10):
        ...
        if tab == 9:
            ...
            index = -2
        plt.plot(X,weight__new[0][index])
